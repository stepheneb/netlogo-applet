...
Globals [scale eRadK gHP mOffP ixP iyP iSizeP cycleCount rX rY rVX rVY rMass b1 b2 b3 b4 mX mY alive? Gm Gmm running? oldLS oldMH oldIScale launchHeight time nextPuffTime puffInterval fuelUsed mountainHeight iscale]
...
to updateUI                           ;; this adjusts scene for changes in arrows, scales, and mountain height
  ...
  if not (oldMH = mountainHeight)     ;; rescale the mountain if its Height is changes
     [set oldMH mountainHeight
     ask mountains [
       let earthTopP min-pycor + gHP ;; the top of the earth in the main scene in patchs 
       let mountainSizeP mountainHeight * scale ;; measured in patchs
...
to initialize
  ...
  set mountainHeight  8.8            ;; the height of the mountain in km
  set launchHeight 350               ;; the height of the launching arrow in km
  ...
  create-ISSs 2 [set size 0 set shape "ISS" ]  ;; the ISS will show up in the main scene and the insert
...
to placeArrows ;; keep the arrows up-to-date with changes in the sliders
  ...
  ask arrow 3 [
    set heading 90
    setxy ixP iyP + iScale * (launchHeight + eRadK)
    set size 8 ]
end
...
to launch  ; move the ISS to the launcher and give it an initial velocity
  ...
  set fuelUsed 0                ;; reset measure of fuel used
  set puffInterval .04 * max-pxcor / (scale * 7.7) ;; sets 50 puffs across screen given launch speed
end
...
to showresults
  ...
  output-type "Fuel Used: "
  output-type round fuelUsed
  output-print  "kNs"
end
...
to fire
  ask ISSs [
    set rvx rvx + 1000 * (thruster_impulse * sin heading) / rMass  ;; rmass is the ISS mass 
    set rvy rvy + 1000 * (thruster_impulse * cos heading) / rMass ] ;; the 1000 factor is because the slider is in kMs
  set fuelUsed fuelUsed + thruster_impulse 
end
...
to placeISSs   ;; show the ISSs at their location rx,ry if visible
   if alive? [
     let relAngle 270
     ask ISSs
       [ifelse ISS_Direction = "Forward" 
         [ set relAngle 0 ]
         [ifelse ISS_Direction = "Right" 
           [ set relAngle 90 ]
           [if ISS_Direction = "Backward" 
             [ set relAngle 180 ]]]
        set heading atan rvx rvy  + relAngle ]
     ask ISS 0 [                 ;; if it is visible in the main scene, show it
     ...
     ask ISS 1 [   ;; if it is visible in the insert, show it. 
       let temp insert rx ry
       ifelse last temp             ;; if it is visible in the insert.......
       [setxy first temp item 1 temp               ;; update the location of the ISS
          set size 8 ]        ;; turn it on in case it has been off
    ...
    [ let temp main mX mY          ;; get screen coordinates of moon
      ifelse last temp             ;; check whether visible
        [setxy first temp item 1 temp  ;; move the moon to location 
          set size 2 * 1737 * scale    ;; 1737 is the moon radius
...
224
451
Challenge:\n\nThe ISS needs to be decomissioned.\n\nShould NASA send it to deep space or the oceans? \n\nFind the least amount of energy required for each fate. 
11
104.0
1
...
----------------------
Developed by Bob Tinker May 2010
