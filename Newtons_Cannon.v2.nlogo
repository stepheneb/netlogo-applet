...
to placeArrows ;; keep the arrows up-to-date with changes in the sliders
  ask arrows [
    set heading Launch_Angle ;; aim it in the right direction
...
to launch  ; move the ISS to the launcher and give it an initial velocity
  set rVX launchSpeed * sin Launch_Angle
  set rVY launchSpeed * cos Launch_Angle
...
WHAT IS IT?
-----------
This is a simulation of Newton's Cannon. Newton imagined a cannon so powerful that it could send a cannonball all the way around the earth. He used this thought experiment to show that gravity was universal; that the gravity that holds us to Earth is the same that causes planets to orbit the sun. 

In this model and arrow represents the launcher sitting on top of a mountain the height of Mount Everest. It launches an odd-shaped thing that is supposed to be the International Space Station (ISS). 

Can you find a combination of launch speed and angle that will result in a circular orbit? Can you get the ISS to hit the back of the arrow? 

Note, there is no air friction in this model. 


THINGS TO TRY
-------------
Pick a starting velocity and then play with the launch angle to get the largest range (distance traveled). If you are systematic, you will see an interesting trend in the angle as velocity increases in steps of 1 km/s. 

Note, to see how far the ISS goes before crashing, you need to change the scale slider. (Ignore the numbers on the slider)



HOW IT WORKS
------------
The ISS is subject to the universal law of gravitation. The force on it is 
G*me*m/r^2
The direction of the force is toward the center of the Earth. 
G is the universal gravitational constant
me is the mass of the earth (in kg)
m is the mass of the ISS (in kg)
r is the distance from the center of the Earth (in km)
In these units the product G*me is 398,600

In the calculation, the vector F is represented as (Fx,Fy), the vector v is (vx,vy)
and the vector r is (x,y), the distance to the center of the earth. 
r = (x^2 + y^2)^.5
ax = (G*me)*x/r^3 and 
ay = (G*me)*y/r^3

The ISS is moved forward a time step dt (seconds) using a half-step method
Starting at (x0,y0) and velocity (vx0,vy0) a half step postion is computed as
xh = x0 + vx0*dt/2
yh = y0 + vy0*dt/2
Then the acceleration (axh,ayh) there is computed
The new velocity is 
vx1 = vx0 + axh*dt
vy1 = vy0 + ayh*dt
...
----------------------
Developed by Bob Tinker in May 2010.
