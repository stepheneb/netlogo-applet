...
to startup 
  ...
  create-stoplights 1 [         ; this tells whether the model is running or not
    set size 2
    set shape "circle"
    set color red
    setxy max-pxcor - 1 min-pycor + 1 ]  ; place it in the upper left corner
...
to go            ; this is the main loop--it switches, depending on mode
  ...
  ifelse mode = 0 and running?           ; set stoplight green if the model is running. 
    [ask stoplights [set color green set label "Computing    "]]
    [ask stoplights [set color red set label "Paused    "]]
end
...
to run-model  
  every .01  [         ; run this every .01 sec. 
    if running?  [ 
      ...
      move-drops
      move-dots]]
  every .02 [if running? [save-state]]  ; record the state of the system
end
...
;  let logicals list combine-event? merge-event?    
  let frame (list surface drop-info dot-info)    ; create a list of all the information on a frame. 
...
to re-run                              ; in mode 2 this is called from the main loop
  every .02 [                          ; every .02 sec 
    ...
    ifelse (rerun-index + 2 >= (length history)) 
      [ restart-model set mode 0 ]     ; switch to compute if the last state in history has been restored
...
WHAT IS IT?
-----------
The huge oil spill in the Gulf of Mexico brought international attention to efforts to clean up the spill. The most effective approach involved using detergents exactly like the liquid soaps you use to clean dishes. Injecting detergents right into the plume of escaping hot oil was more effective than expected. Although the long-range impact is still being debated, the detergents may have substantially reduced the environmental impact of the oil, giving oil-eating bacteria time to digest the waste oil. 
This is a model of detergents that demonstrates the molecular properties of detergents and why they are effective at dispersing the oil. The principles used in making detergents are important throughout chemistry and biology because they determine which molecules dissolve in water and which avoid water. Once you understand what is happening at the molecular level, you can guess why detergents were unexpectedly effective. 

HOW TO USE IT
-------------
Turn the model on by pressing the ÒOnÓ button in the upper left. This is like a power switchÑleave it on because nothing happens when it is off. 
The model starts with a layer of yellow oil floating over blue water. Nothing happens until you press the ÒShakeÓ button. Each time you press it, a new oil droplet is formed that drifts down from the oil layer. 
Three mystery molecules are available. You can select black, magenta, or green molecules using the pull-down menu. Click on the oil or water to inject these molecules into the mix. 
You can always see what the molecules look like by clicking on the ÒShow Mystery MoleculesÓ button, but that stops the model. Black stringy molecules are hydrocarbons that do not dissolve in water, red charged atoms are attracted to the highly polarized environment of water.  To continue running the model, press the ÒHide Mystery MoleculesÓ button. 
At any time, you can review the earlier history of the model by click-dragging on the ÒTape-positionÓ slider. You can step forward or backward by clicking on the dark green part of the slider. You can also click on the ÒRe-runÓ button to run the model from wherever you placed the slider. If you do this, the slider will move toward 100% and then the software will switch from showing old data to computing new motions. If you look closely at the lower right of the model window, you will see an indication of whether the model is showing old data or generating new data. 
If the model seems to be slowing down, you should press the ÒResetÓ button. The slow-down due to the enormous amount of data that are needed to save the motion of the model. ÒResetÓ eliminates the data, so donÕt press it until you are sure you will not need it. 

THINGS TO TRY
-------------
Your goal is to eliminate the oil layer by some combination of shaking and injecting the right molecules in sufficient number. 
Experiment first by shaking the oil. Observe the oil and note what happens to the oil droplets. Can you shake it hard enough to eliminate the oil layer? Does the layer return? 
Next, add green and magenta molecules and shake the mix. Observe where these molecules go. Before looking at the molecules, predict which type are hydrocarbons and which are ions. Do these molecules have any effect on the oil layer. 
Now add the black molecules and shake. Where do these go? Predict their structure and then check your prediction. Can you explain why most of them end up where you see them? 
Try adding lots of molecules and shaking. Can you get the oil layer to go away and stay away? What molecules are responsible for this? Why does this happen? 

IN SUMMARY
-------------
Given what you learned about detergents, what is it about the way detergent was used in the Gulf might explain their unexpcted effectiveness? 

CREDITS AND REFERENCES
----------------------
Copyright 2011 (c) the Concord Consortium. You can use this for any purpose, providing you follow the LGPL license. 
Designed, programmed, and annotated by Robert Tinker for the VISUAL project. Design assistance by David Miller. 

WHAT IS IT?
-----------
The tragic oil spill in the Gulf of Mexico brought international attention to efforts to clean up the spill. The most effective approach involved using detergents exactly like the liquid soaps you use to clean dishes. Injecting detergents right into the plume of escaping hot oil was more effective than expected. Although the long-range impact is still being debated, the detergents may have substantially reduced the environmental impact of the oil, giving oil-eating bacteria time to digest the waste oil. 
This is a model of detergents that demonstrates the molecular properties of detergents and why they are effective at dispersing the oil. The principles used in making detergents are important throughout chemistry and biology because they determine which molecules dissolve in water and which avoid water. Once you understand what is happening at the molecular level, you can guess why detergents were unexpectedly effective. 
...
IN SUMMARY
-------------
Given what you learned about detergents, what is it about the way detergents were used in the Gulf that might explain their unexpecte effectiveness. 
